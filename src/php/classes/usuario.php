...
class Usuario {
    ...
    /**
     * Retorna informações do usuário pelo usuário
	...
	public function setSenha($senha) {
		if(!is_string($senha)) {
			throw new InvalidArgumentException("Erro ao definir a senha do usuário. Esperava uma string, recebeu ".gettype($senha).Utilidades::debugBacktrace(), E_USER_ERROR);
		}
		else if(is_null($senha)) {
			throw new InvalidArgumentException("Erro ao definir a senha do usuário. String nula".Utilidades::debugBacktrace(), E_USER_ERROR);
		}
		else {
			$this->senha = $senha;
		}
